 ...
 *      ========
 *
 * version 6, 25-Feb-2007
 *
 * reads lines from sfdfile and creates commands to insert into sfdb database
...
int main( int argc, char *argv[] )
{
	...
	char     ignstr[cBcShortStrLth];    /* ignore string */
	...
	TSyBoolean noinsert;                /* produce no insert code */
	TSyBoolean noignore;                /* produce no ignore keyword code */
	TSyBoolean invhdr;                  /* invert header info in sfd entry */
	TSyBoolean baynet;                  /* convert SH to BH for Bayernnetz */
	...
	noinsert = pa_qspecified( "-noins" );
	noignore = pa_qspecified( "-noign" );
	invhdr = pa_qspecified( "-invhdr" );
	baynet = pa_qspecified( "-baynet" );

	if  (noignore)  {
		*ignstr = '\0';
	} else {
		strcpy( ignstr, "ignore " );
	} /*endif*/
	...
	if  (pathid == -1)  {
		...
		sprintf( shellcmd,
			"\\rm %s; %s \"select id from pathtab where rootpath = \'%s\'\" >%s",
	...
	while  (fgets(line,cBcLineLth,fp) != NULL)  {
		...
		if  (baynet)  {
			if  (strncmp(descr.stream,"rjob-sh",7) == 0)  descr.stream[5] = 'b';
			if  (strncmp(descr.stream,"manz-sh",7) == 0)  descr.stream[5] = 'b';
			if  (strncmp(descr.stream,"rotz-sh",7) == 0)  descr.stream[5] = 'b';
		} /*endif*/
		...
		if  (!noinsert)
			fprintf( out,
				"insert %sinto sftab values ( \"%s\", \"%s\", \"%c\", \"%d\", \"%s\", \"%4d%02d%02d\", \"%2d%02d%02d.%03d\", \"%4d%02d%02d\", \"%2d%02d%02d.%03d\", \"%d\", \"%d\", \"%d\", \"%d\", \"%d\", \"%d\", \"%d\", NULL );\n",
				ignstr, station, chan, comp, pathid, rpath, stime.year, stime.month, stime.day,
		...
		if  (update)
			fprintf( out,
				"update %ssftab set sdate=\"%4d%02d%02d\", stime=\"%2d%02d%02d.%03d\", edate=\"%4d%02d%02d\", etime=\"%2d%02d%02d.%03d\", recnum=\"%d\", dataflags=\"%d\", priority=\"%d\", dataformat=\"%d\", hswap=\"%d\" where pathid=\"%d\" and relpath=\"%s\";\n",
				ignstr, stime.year, stime.month, stime.day, stime.hour, stime.min,
				stime.sec, stime.ms, etime.year, etime.month, etime.day, etime.hour,
				etime.min, etime.sec, etime.ms, descr.recno, descr.dataflags,
				priority, dataformat, descr.swap_hdr, pathid, rpath );
		sfcnt++;
