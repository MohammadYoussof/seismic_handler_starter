 ...
 *      ===========
 *
 * version 11, 22-Dec-2003
 *
 * maps matrix
 * K. Stammler, 22-Jun-94
 */
...
/* global variables */
static char		*mmv_vec[]={"mapmatrix.uid"};   /* MRM database file list */
...
int main( int argc, char *argv[] )
{
	...
	BOOLEAN           square_it;        /* square input data */
	...
	if  (pa_pnumber() != 3)  {
		fprintf( stderr, "*** Usage: %s <file> <colnum> <scale> ***\n", argv[0] );
	...
	square_it = FALSE;
	if  (pa_qspecified("-square"))  square_it = TRUE;
	mmv_cartesian = FALSE;
	...
	*mmv_abstime = '\0';
	...
	/* read in data file */
	mm_read_matrix( inpfile, square_it, &mmv_matrix );
